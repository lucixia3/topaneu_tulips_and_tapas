...
def laterality_from_vessel(vessel_name: str) -> str:
    """R/L from the vessel prefix"""
...
def assign_component(coords_mm: np.ndarray, tree: cKDTree,
                     vessel_classes: np.ndarray) -> dict[str, float]:
    ...
    for cls, w in zip(nn_classes.tolist(), weights.tolist()):
        weight_by_class[cls] = weight_by_class.get(cls, 0.0) + w
    winning_class = max(weight_by_class, key=weight_by_class.get)

    return {
        "vessel_value": int(winning_class),
        "min_dist_mm": float(distances.min()),
        "vote_confidence": weight_by_class[winning_class] / total_weight if total_weight > 0 else 0.0,
    }


def assign_vessel_location(aneurysm_arr: np.ndarray, vessel_arr: np.ndarray,
                           affine: np.ndarray, min_voxels: int,
                           max_dist_mm: float) -> list[dict[str, object]]:
    ...
    for component_id, comp_label in enumerate(kept, start=1):
        ...
        result = assign_component(coords_mm, tree, vessel_classes)
        vessel_value = int(result["vessel_value"])
        vessel_name = VESSEL_LABELS.get(vessel_value, f"UNKNOWN-{vessel_value}")

        location_name = VESSEL_TO_LOCATION.get(vessel_name)
        if location_name is None:
            unmapped.add(vessel_name)
            location_value: Optional[int] = None
            location_name = ""
        else:
            location_value = LOCATION_LABELS.get(location_name)
        ...
        rows.append({
            ...
            "nearest_vessel_value": vessel_value,
            "location_class": location_name,
            "location_value": "" if location_value is None else location_value,
            "laterality": laterality_from_vessel(vessel_name),
            "min_dist_mm": round(float(result["min_dist_mm"]), 4),
            "vote_confidence": round(float(result["vote_confidence"]), 4),
            "flagged": bool(result["min_dist_mm"] > max_dist_mm),
        })
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="assign each aneurysm the location of its nearest vessel "
                    "(topaneu 2026 stage 2 baseline).")
    ...
    print(f"wrote {len(rows)} row(s) to {args.out} ({n_flagged} flagged).")
    return 0
...
"""
 python assign_vessel_location.py `
