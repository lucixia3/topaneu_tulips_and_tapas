...
class Trainer():
    ...
    def train(self, model, train_dl, val_dl, epochs=1, early_stop=5, wdir=Path(datetime.datetime.now().strftime(r'TnTS2_training_from-%H:%M:%S-%d.%m.%y'))):
        ...
        model.to(torch.half).to(self.device)
        ...
        for e in range(1, epochs+1):
            ...
            for batch in tqdm.tqdm(train_dl, desc='Training batches'):
                ...
                l = model.loss(batch['image'].to(torch.half).to(self.device), batch['coords'].to(torch.half).to(self.device), batch['modality'], batch['location'].to(torch.half).to(self.device))
                l.backward()
                self.optim.step()
