...
class NFoldTrainer():
    ...
    def train(self, model, ds, train_trans, val_trans, epochs=1, early_stop=5, wdir=Path(datetime.datetime.now().strftime(r'TnTS2_Nfold_training_from-%H:%M:%S-%d.%m.%y')), use_aneu_class_balancing=False):
        os.makedirs(wdir)
